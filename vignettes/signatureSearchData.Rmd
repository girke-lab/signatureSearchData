...
```{r ah_explore_ssd1, eval=TRUE, warning=FALSE, message=FALSE }
...
```{r ah_explore_ssd2, eval=TRUE, warning=FALSE, message=FALSE }
ssd$title
```
...
```{r ah_explore_ssd3, eval=TRUE}
as.list(ssd)[10]
```

Details about the usage of `AnnotationHub` can be found in its vignettes [here](http://bioconductor.org/packages/AnnotationHub/).
