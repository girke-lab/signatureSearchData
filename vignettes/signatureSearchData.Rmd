...
subsets of manageable size and then appended to an HDF5 file. In the following,
the latter is referred to as the LINCS database. Since the size of the full
matrix is tens of GBs in size, it would consume too much memory to be read into
R at once. Reading the matrix in smaller batches and appending them to an HDF5
file is much more memory efficient. 

We also provide the utility to import the data stored in the HDF5 file as a
...
gene expression values across for all treatmens and cell lines used by LINCS.
The Level 3 signatures were filtered the same way as the Level 5 data. Since
Level 3 data contains the results from biological replicates, they were
collapsed to mean values. Subsequently, the resulting matrix of mean expression
...
treatments and 30 cell lines. <font color="blue">[Need to explain why the
number of signatures and compounds is lower for Level 3 than Level 5. Otherwise
this is very confusing.]</font> 
