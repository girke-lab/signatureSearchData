...
way one can identify knockouts, diseases or drugs that induce expression phenotypes 
similar to a sample of interest. Currently, `signatureSearchData` includes
...
lines [@Lamb2006-du; @Subramanian2017-fu]. Here these data sets have been
preprocessed as required for the gene expression signature search (GESS)
algorithms implemented and used by `signatureSearch`. The preprocessed data
types include normalized gene expression values (_e.g._ intensity values), log
fold changes (LFC), P or FDR values for DEG calls, rankings based on some of
these preprocessing outcomes or sets of top up/down-regulated DEGs. 
...
`signatureSearchData` does not include RNA-Seq data mainly because of the lack
...
for download. Since the next paragraph addresses this, it could be moved here to 
address this. </font>
