...
The LINCS data is generated from L1000 assay. It directly measures the expression 
of 978 landmark genes and 80 control genes by detecting fluorescent intensity of beads 
after capturing the amplification products of mRNAs [@Peck2006-rf]. The expression 
of 11,350 additional genes is imputed from the landmark genes by using as training 
data a collection of 12,063 Affymetrix gene chips [@Edgar2002-di].

The LINCS data are pre-processed at 5 different levels and deposit at GEO. 
Level 1 data are the raw mean fluorescent intensity values that come 
...
applied to the replicated samples of each experiment (generally 3 replicates) to 
compute a weighted average signature (Level 5). For more detailed description of 
the preprocessing methods used by the LINCS, readers want to refer to the 
[LINCS user guide](https://docs.google.com/document/d/1q2gciWRhVCAAnlvF2iRLuJ7whrGP6QjpsCMq1yWz7dU/edit#).

LINCS level5 dataset contains 473,647 signatures from 19,811 compound treatment in different 
...
the concentration and duration information was ignored by only selecting those 
signatures at specific concentration (10 um) and duration (24 h) since most drugs 
have this dosage and time point. Since not all compounds have the selected concentation
and duration, 8,104 compounds were selected in 30 cells. Since one compound has treatments
in multiple cells, the total number of selected signature is 45,956. 

So, the `lincs` database represents moderated z-scores from DE analysis of 12,328 
genes from 8,140 compound treatments in 30 cells. It can be used for all the GESS 
methods in the `signatureSearch` package.
...
directory of the user’s R session. The `data` direcotry is used to store the 
input data as convention. All paths in this vignette are given relative to the present working 
