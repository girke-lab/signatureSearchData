...
The following selects LINCS Level 5 signatures of compound treatments at a concentration of 10 uM 
and a treatment time of 24 hours.
...
Next the large z-score matrix of expression signatures is imported step-wise in subsets of
manageable size and then appended to an HDF5 file. In the following, the latter is referred to as the
LINCS database. Since the size of the full matrix is tens of GBs in size, it
would consume too much memory to be read into R at once. Reading the matrix in
smaller batches and appending them to an HDF5 file is much more memory efficient. 

We also provide the utility to import the data stored in the HDF5 file as a `SummarizedExperiment` object.
