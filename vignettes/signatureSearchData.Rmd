...
The LINCS2 dataset contains moderated z-scores from DE analysis
...
reference database used for the Query Tool in CLUE website. Like the LINCS 
database, users have the option to assemble any custom collection 
...
vector called `lincs2_path`. A summary of the content of the HDF5 file can be returned with
the `h5ls` function. Note, due to the large size of the LINCS data set, its download 
takes too much time to evaluate the following code section during the build time of 
this vignette. 
